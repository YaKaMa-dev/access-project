@extends('welcome')
@section('pageTitle', 'Projets / Create Accesses')
@section('content')
@if ($errors->any())
...
<div class=" flex items-center justify-center py-8 px-6 sm:px-6 lg:px-10">
    ...
    <div class="max-w-full w-full space-y-6">
      ...
      <div class="rounded-2xl  bg-white max-w-full  overflow-hidden shadow-xl p-8">
        ...
        <form class="space-y-4" action="{{route('projets.accesses.store',$projet->id)}}" method="POST">
            @csrf            
            <div class="flex flex-wrap -mx-3 mb-6">
                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                    <label for="project_name" class="block font-medium text-lg text-gray-700">Username</label>
                    <input name="username" value="{{old('username')}}" type="text" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded-md py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                </div>
            ...
            <a href="{{route('projets.show',$projet->id)}}" class="md:inline relative md:w-1/6 w-full  flex justify-center py-2 px-4 border border-black text-sm font-medium rounded-md text-black bg-white-600 hover:bg-black hover:text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-black-500 mt-2">back</a>
          </div>
        </form>
    </div>
  </div>
